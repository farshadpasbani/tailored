...
describe.skipIf(!canRun)("tailored legacy-fit CLI", () => {
  it("verdicts APPLY for the example candidate against the example jd", () => {
    const r = run(["legacy-fit", "--jd", `${ex}/jd.yaml`, "--canon", `${ex}/canon.yaml`]);
  ...
  it("verdicts SKIP and exits 1 when must-have coverage is below the floor", () => {
    ...
    const r = run(["legacy-fit", "--jd", bad, "--canon", `${ex}/canon.yaml`]);
  ...
  it("fails fast when --floor exceeds --apply", () => {
    const r = run(["legacy-fit", "--jd", `${ex}/jd.yaml`, "--canon", `${ex}/canon.yaml`, "--apply", "0.5", "--floor", "0.9"]);
    expect(r.code).toBe(1);
